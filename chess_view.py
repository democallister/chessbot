...
class View():
    ...
    def start_game(self, event=None):
        builtins.cfg = chess_config.Config(self.board_type_var.get().lower(),
        ...
        self.draw_board()
        self.draw_pieces()
        if self.game_type_var.get() == 'Human v Human':
            ...
            game_type_text = ('Human versus ' + 
                self.opponent1_type_var.get().replace('_ChessPlayer',''))
        else:  # Computer v Computer
            ...
            game_type_text = (
                self.opponent1_type_var.get().replace('_ChessPlayer','') + 
                ' versus ' + 
                self.opponent2_type_var.get().replace('_ChessPlayer',''))
